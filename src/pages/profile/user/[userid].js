...
export default function Profile() {
    ...
    useEffect((userinfo) => {
        ...
        const requestData = async () => {
          ...
          try {
            ...
            const res = await axios.get(`http://localhost:8800/profile/user/${userinfo.id}`,{params:{id:userinfo.id}});
            setInfo(res.data);
  ...
  <div class="desc">
        <p>Имя пользователя: {info.username}</p>
        <p>Почта: {info.email}</p>



  </div>
  <button className='send-button' onClick={()=>router.push(`/profile/${userinfo.id}`)} >Редактировать профиль</button>
  </div>

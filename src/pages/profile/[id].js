...
const [error,setError] = useState(false)
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values)=> {
          ...
          try {
            await axios.post(`http://localhost:8800/profile/${userinfo.id}`, values);
            router.push(`/profile/user/${userinfo.id}`)
    ...
    useEffect((userinfo) => {
        ...
        const requestData = async () => {
          ...
          try {
            ...
            const res = await axios.get(`http://localhost:8800/profile/user/${userinfo.id}`,{params:{userid:userinfo.id}});
            setInfo(res.data);
...
<div className='desc'>
  ...
  {info&&info.map((i)=>{
    return(
      <>
      <form style={{display:'flex', flexDirection:'column',gap:20,marginLeft:10}} onSubmit={formik.handleSubmit}>
<input name='fio'  className='edit-input' placeholder={i.fio}  onChange={formik.handleChange} onBlur={formik.handleBlur}
        value={formik.values.fio}/>


<input name='username'  className='edit-input' placeholder={i.username} onChange={formik.handleChange} onBlur={formik.handleBlur}
        value={formik.values.username}/>
{formik.errors.username ? <p className="error" >
                  {formik.errors.username }
                </p>:null}
<input name='email' className='edit-input' placeholder={i.email}  onChange={formik.handleChange} onBlur={formik.handleBlur}
...
<button className='send-button' onSubmit={formik.handleSubmit} >Отправить</button>
